...
export function ProjectDashboard({ projects }: { projects: ProjectRecord[] }) {
  ...
  return <section className="mt-8" aria-labelledby="dashboard-title">
    ...
    <div className="mt-7 grid gap-6 lg:grid-cols-2"><Agenda title="Overdue" description="Items that need attention now." items={overdue} empty="Nothing overdue." danger onOpen={setEditingId} /><Agenda title="Upcoming" description="The next dated commitments across your local portfolio." items={upcoming} empty="No upcoming dated items." onOpen={setEditingId} /></div>
...
function Agenda({ title, description, items, empty, danger, onOpen }: { title: string; description: string; items: AgendaItem[]; empty: string; danger?: boolean; onOpen: (projectId: string) => void }) {
  return <section className="border bg-[var(--card)] p-5" aria-labelledby={`${title.toLowerCase()}-title`}><h3 id={`${title.toLowerCase()}-title`} className="font-semibold">{title}</h3><p className="mt-1 text-sm text-[var(--muted-foreground)]">{description}</p><div className="mt-4 grid gap-2">{items.length === 0 ? <p className="border border-dashed p-3 text-sm text-[var(--muted-foreground)]">{empty}</p> : items.map((item) => <article key={item.id} className="flex items-center justify-between gap-3 border p-3"><div className="min-w-0"><button type="button" className="truncate text-left text-sm font-medium underline-offset-4 hover:underline focus-visible:outline-2 focus-visible:outline-[var(--ring)]" onClick={() => onOpen(item.projectId)}>{item.title}</button><p className="mt-1 truncate text-xs text-[var(--muted-foreground)]">{item.project} · {item.kind}</p></div><time dateTime={item.date} className={`shrink-0 text-xs font-medium ${danger ? "text-[var(--destructive)]" : ""}`}>{item.date}</time></article>)}</div></section>;
}
