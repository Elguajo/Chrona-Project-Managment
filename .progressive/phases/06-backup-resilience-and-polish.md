...
- Restore stages cover assets, swaps the cover directory with rollback protection, and replaces SQLite rows in one transaction. Domain coverage restores Projects, Tasks, Documents, status history, and templates; it rejects both an unsupported schema version and a Task referring to a missing Project while confirming the prior database remains intact.
